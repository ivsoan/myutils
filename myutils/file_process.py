...
def save_dict_to_npz_file(data: dict, save_path: str, name: str, logger: logging.Logger = None) -> str:
    log = logger.info if logger else print
    if not os.path.exists(save_path):
        os.makedirs(save_path)
        log(f"Created directory: {save_path}.")

    output_file = os.path.join(save_path, f"{name}.npz")
...
def load_npz_file(file_path: str, logger: logging.Logger = None) -> dict | None:
    log_error = logger.error if logger else print
    try:
